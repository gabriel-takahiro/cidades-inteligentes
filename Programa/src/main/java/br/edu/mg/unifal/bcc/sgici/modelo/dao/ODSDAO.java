...
public class ODSDAO {
	...
	public void cadastrarODS(int numero_ods, String nome_objetivo) {
		try {
			...
			try (PreparedStatement pstm = connection.prepareStatement(sql)) {
				...
				new JanelaMensagem("A ODS " + numero_ods + " foi cadastrada com sucesso.");
			}
		} catch (Exception e) {
	...
	public void atualizarODS(String nome_objetivo, int numero_ods_atualizar) {
		try {
			...
			try (PreparedStatement pstm = connection.prepareStatement(sql)) {
				...
				JanelaPrincipal.atualizarODS();
				new JanelaMensagem("A ODS " + numero_ods_atualizar + " foi atualizada com sucesso!");
			}
		} catch (Exception e) {
	...
	/**
	 * Exclui da tabela "ods" uma ODS a partir do número da ODS e do nome do
	 * objetivo
	 * 
	...
	public void excluir(ODS ods) {
		try {
			...
			throw new RuntimeException("Não foi possível excluir a(s) ODS devido à seguinte razão:\nA ODS "
					+ ods.getNumero_ods()
					+ " está sendo atualmente utilizada por pelo menos uma meta. Por favor, remova a ODS das metas em questão antes de tentar excluí-la novamente.");
		}
	}

...
public class CalculoIndicadorDAO {
	 ...
	 * @param codigoMunicipio              código do município a ser buscado
	 * @param data                         ano a ser buscado
	 * @param listaIndicadoresSelecionados lista de indicadores selecionados no
	 *                                     programa
				...
				try (ResultSet rst = pstm.getResultSet()) {
					...
					for (Integer codigo : listaIndicadoresSelecionados) {
						...
						for (IndicadoresBuscados indicadoresBuscados : resultados) {
							if (indicadoresBuscados.getCodigo_indicador() == codigo) {
								valor++;
							}
						}
						if (valor == 0) {
							listaCodigoIndicadores.add(codigo);
						}
					}
	 ...
	 * @param valor_oficial    indicador foi calculado apenas com valores dos bancos
	 *                         de dados oficiais?
	 */
	...
	/**
	 * Busca todos os indicadores calculados para todos os municípios em um
	 * determinado ano
	 * 
	...
	/**
	 * Busca na base de dados local os indicadores selecionados que ainda não foram
	 * calculados para o município e data especificados nos parâmetros. O método
	 * realiza uma consulta SQL para cada indicador selecionado na lista,
	 * verificando se já existe um cálculo de indicador para o município e data
	 * informados. Caso não exista, o código do indicador é adicionado a uma lista
	 * de indicadores não calculados.
	 * 
	 * @param listaIndicadoresSelecionados a lista de códigos dos indicadores
	 *                                     selecionados
	 * @param parametros                   os parâmetros de busca, contendo o código
	 *                                     do município e o ano
	 * @return uma lista de códigos dos indicadores selecionados que ainda não foram
	 *         calculados para o município e data especificados
	 */
	public List<Integer> buscaIndicadoresNaoCalculados(List<Integer> listaIndicadoresSelecionados,
			ParametrosBuscaValorVariavel parametros) {
		try {
			String sql = "SELECT codigo_indicador FROM calculo_indicador WHERE codigo_indicador = ? AND codigo_municipio = ? AND data = ?";
			try (PreparedStatement pstm = connection.prepareStatement(sql)) {
				List<Integer> listaIndicadoresNaoCalculados = new ArrayList<>();
				for (Integer codigo : listaIndicadoresSelecionados) {
					pstm.setInt(1, codigo);
					pstm.setInt(2, parametros.getCodigoMunicipio());
					pstm.setString(3, parametros.getData());
					pstm.execute();
					try (ResultSet rst = pstm.getResultSet()) {
						if (!rst.next()) {
							listaIndicadoresNaoCalculados.add(codigo);
						}
					}
				}
				return listaIndicadoresNaoCalculados;
			}
		} catch (Exception e) {
			System.out.println("Erro na buscaIndicadoreNaoCalculados" + e);
			throw new RuntimeException();
		}
	}

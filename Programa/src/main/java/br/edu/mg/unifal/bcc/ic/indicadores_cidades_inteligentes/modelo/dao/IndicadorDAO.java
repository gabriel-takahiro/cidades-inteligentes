...
public class IndicadorDAO {
	...
	public List<Indicador> buscarCodigoEMetodo(List<Integer> listaIndicadores) {
		...
		try (PreparedStatement pstm = connection.prepareStatement(sql)) {
			...
			throw new RuntimeException("Falha ao recuperar codigo e metodo");
		}
	}
								...
								try (ResultSet rstVariaveisValor = pstmVariaveisValor.getResultSet();) {
									if (rstVariaveisValor.next()) {
										String[] linha2 = { "", "", "",
												...
												rstVariaveisValor.getString(1), Boolean.toString(rstVariaveisValor.getBoolean(3)),
	...
	public void excluir(int codigo) {
		try {
			...
			throw new RuntimeException("Falha ao excluir o indicador: " + codigo + "\nEste indicador este indicador possui resultados.");
		}
	}
	...
	public List<Integer> buscaIndicadoresComoVariaveis(List<Integer> listaIndicadoresSelecionados) {
		List<String> stringIndicadores = listaIndicadoresSelecionados.stream().map(Object::toString).collect(Collectors.toList());
		String indicadores = String.join(",", stringIndicadores);
		String sql = "SELECT codigo_indicador, nome_indicador, numero_meta, metodo_calculo, padrao FROM indicador WHERE codigo_indicador IN ("
				+ indicadores + ")";
				...
				while (rst.next()) {
					if(rst.getString(4).contains("{")) {
						...
						while(metodoCalculo.contains("{")) {
							int indicador = Integer.parseInt(metodoCalculo.substring(metodoCalculo.indexOf("{")+1, metodoCalculo.indexOf("}")));
							metodoCalculo = metodoCalculo.substring(metodoCalculo.indexOf("}")+1, metodoCalculo.length());

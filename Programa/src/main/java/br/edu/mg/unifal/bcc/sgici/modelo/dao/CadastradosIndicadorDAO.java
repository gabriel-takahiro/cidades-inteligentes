...
public class CadastradosIndicadorDAO {
	...
	public void cadastrarIndicador(String nomeIndicador, String metodoCalculo, String eixo, String tipoPlano,
		...
		try {
			...
			try (PreparedStatement pstm = connection.prepareStatement(sql)) {
				...
				new JanelaMensagem("O indicador foi cadastrado com sucesso.");
			}
		} catch (Exception e) {
			throw new RuntimeException(
					"Falha ao cadastrar o indicador. Já existe um indicador com o mesmo método de cálculo.");
		}
	}
	 ...
	 * @param padrao               indicador é padrão, ou seja, veio junto com o
	 *                             programa?
	 */
	public void atualizarIndicador(int codigo_indicador, String nome_indicador, String metodo_calculo, String eixo,
		...
		try {
			...
			try (PreparedStatement pstm = connection.prepareStatement(sql)) {
				...
				JanelaPrincipal.atualizarIndicadores();
				new JanelaMensagem("O indicador " + codigo_indicador + " foi atualizado com sucesso!");
			}
		} catch (Exception e) {
			if (e instanceof PSQLException && e.getMessage().contains("violates unique constraint")) {
				throw new RuntimeException("Não foi possível atualizar o indicador de código " + codigo_indicador
						+ ".\nO método de cálculo informado já está em uso por outro indicador. Por favor, escolha um método de cálculo diferente.");
			}

...
public class Divisao extends Calculo {
	...
	public Divisao(Calculo proximo) {
		super(proximo);
	}
	...
	public CompostoCalculo efetuarCalculo(CompostoCalculo compostoCalculo) {
		...
		if (seq.get(posicao + 1).equals("[")) { // Se o próximo elemento for um abre
			...
			if (posicao == -1) {
				deuCerto = false;
				posicao = seq.size();
				mensagemErro = "Erro na divisão ao tentar calcular a prioridade com o próximo elemento sendo um abre parênteses";
			}
		} else {
			try {
				...
				deuCerto = false;
				posicao = seq.size();
				mensagemErro = "Erro na divisão ao tentar buscar o valor da variável" + e;
			}
			posicao++;
		}
